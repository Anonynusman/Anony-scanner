...
# Tool 		 : Anonynusman
# Usage		 : ./scan.py example.com (or) python scan.py example.com
...
if len(sys.argv) == 1 :
    ...
    if target == '--update' or target == '-u' or target == '--u':
        ...
        if oldversion_hash == newversion_hash :
            clear()
            print "\t"+ bcolors.OKBLUE +"You already have the latest version of Anony-scan." + bcolors.ENDC
        else:
            clear()
            print "\t"+ bcolors.OKGREEN +"Anony-scan successfully updated to the latest version." +bcolors.ENDC
        spinner.stop()
        sys.exit(1)
        ...
        if unavail_tools == 0:
        	...
        	print "\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable.Anony-scan can still perform tests by excluding these tools from the tests. Please install these tools to fully utilize the functionality of Anony-scan."+bcolors.ENDC
		...
		rs_skipped_checks = rs_skipped_checks + 1
            ...
            if runTest == 1:
                    ...
                    clear()
                    print "\t"+bcolors.WARNING + "Test Skipped. Performing Next. Press Ctrl+Z to Quit Anony-scan." + bcolors.ENDC
                    rs_skipped_checks = rs_skipped_checks + 1
        ...
        if len(rs_vul_list)==0:
        	...
        	with open("RS-Vulnerability-Report", "a") as report:
        		while(rs_vul < len(rs_vul_list)):
	       		...
	       		print "\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+"`RS-Vulnerability-Report`"+bcolors.ENDC+" is available under the same directory Anony-scan resides."
